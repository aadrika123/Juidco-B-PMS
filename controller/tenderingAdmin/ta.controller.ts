...
export const setUnitPrice = async (req: Request, res: Response) => {
    const result: any = await setUnitPriceDal(req)
    if (!result?.error) {
        res.status(200).json({
            status: true,
        ...
        res.status(400).json({
            status: false,
            message: `Error while adding unit price`,
            error: result?.message
        })
    }
}

export const setComparisonRatio = async (req: Request, res: Response) => {
    const result: any = await setComparisonRatioDal(req)
    if (!result?.error) {
        res.status(200).json({
            status: true,
            message: `Comparison ratio added successfully`,
            data: result
        })
    } else {
        res.status(400).json({
