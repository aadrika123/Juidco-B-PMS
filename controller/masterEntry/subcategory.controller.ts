...
import { createSubcategoryDal, getSubcategoryDal, getSubcategoryByCategoryIdDal, getSubcategoryActiveOnlyDal, switchStatusDal, editSubcategoryDal, getSubcategoryByCategoryIdActiveOnlyDal, getSubcategoryByIdDal } from '../../dal/masterEntry/subcategory.dal'
...
export const getSubcategoryById = async (req: Request, res: Response) => {
    const result: any = await getSubcategoryByIdDal(req)
    if (!result?.error) {
        res.status(201).json({
            status: true,
            message: `Subcategory fetched successfully`,
