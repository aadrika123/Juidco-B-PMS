...
const foreign_wrapper = async () => {
    ...
    await prisma.$queryRaw`CREATE SERVER master_fdw FOREIGN DATA WRAPPER postgres_fdw OPTIONS (host ${process.env.DB_HOST}, port ${process.env.PORT}, dbname ${process.env.DB_NAME})`;
    await prisma.$queryRaw`CREATE USER MAPPING FOR postgres SERVER master_fdw OPTIONS (user ${process.env.DB_USER}, password ${process.env.DB_PASSWORD})`;
