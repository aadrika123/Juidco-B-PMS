...
import { getPreTenderDal } from '../accountant/accPreProcurement.dal'
...
export const getBoqByRefNoDal = async (req: Request) => {
	...
	try {
		...
		const preTenderData = await getPreTenderDal(req)
...
export const getBoqAndPreTenderByRefNoDal = async (req: Request) => {
	const { reference_no } = req.params
	try {
		const result: any = await prisma.boq.findFirst({
			where: {
